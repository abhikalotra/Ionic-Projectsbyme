...
from
    ...
    jos_jakjesc_zamowienie_posilek_godzina h
     ...
     where    z.id_user = '$userId' and 
	 d.dzien = '$pozostalo_dni4' and d.id_posilek_rodzaj = '$breakfastId'";
	...
	$fetchResult = mysql_fetch_array($result, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);	
	  ...
	  //echo "<pre>";print_r($fetchResult['dzien']); die;
			
			if($fetchResult){
				
				$getdata =array('status'=>'Success','mesg'=>'Show Breakfast Details','getDay'=>$getDayNames,'response'=>$fetchResult);	
				}else{
					$underDevelopement = "Under Delevopment Twoje Menu jest w trakcie opracowywania";
	...
	$fetchResult = mysql_fetch_array($result, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);	
			...
			if($fetchResult){
				...
				$getdata =array('status'=>'Success','mesg'=>'Show Breakfast Details','getDay'=>$getDayNames,'response'=>$fetchResult);	
						...
						$getdata = array('status'=>'fail','message'=>'Niestety Twoje zamówienie nie obejmuje dzisiejszego dnia','response'=>$Unfortunately);	
				} 	
	}  
